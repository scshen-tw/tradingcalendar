...
if errorlevel 1 (
    ...
    python log_update_status.py STOCK ERROR "git pull failed" --counts --commit >> update_log.txt 2>&1
    exit /b 1
)
...
if errorlevel 1 (
    ...
    python log_update_status.py STOCK ERROR "update_stocks.py failed" --counts --commit >> update_log.txt 2>&1
    exit /b 1
)
...
if errorlevel 1 (
    ...
    python log_update_status.py STOCK ERROR "git commit failed" --counts --commit >> update_log.txt 2>&1
    exit /b 1
)
...
if errorlevel 1 (
    ...
    python log_update_status.py STOCK ERROR "git push failed" --counts --commit >> update_log.txt 2>&1
    exit /b 1
)

echo [%date% %time%] Stock calendar update completed. >> update_log.txt
python log_update_status.py STOCK SUCCESS "updated and pushed" --counts --commit >> update_log.txt 2>&1
exit /b 0
